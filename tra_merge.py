...
def ParseMergedInsertions(path_input, output_dir):
    ...
    df["SV_ID"] = df['chr'].astype(str) + "_" + df["start"].astype(str) + "_" + df['end'].astype(str)
    df["CHR2"]= df['INFO'].apply(lambda x: extract_info_value(x, 'CHR2'))
    df['END_INFO'] = df['INFO'].apply(lambda x: extract_info_value(x, 'END'))
                                      
    df_ins = pd.DataFrame(columns=['SV_ID', 'Program', 'ProgramID', 'Individual_coord', 'Merged_INS_LEN', 'Individual_INS_LEN'])
    df_ins["SV_ID"] = df['chr'].astype(str) + "_" + df["start"].astype(str) + "_" + df['end'].astype(str)
    df_ins["CHR2"]= df['INFO'].apply(lambda x: extract_info_value(x, 'CHR2'))
    df_ins['END_INFO'] = df['INFO'].apply(lambda x: extract_info_value(x, 'END'))
    
    df_ins["Program"] = df["ID_PROGRAM"].apply(lambda x: x[0])
    df_ins["ProgramID"] = df["ID_PROGRAM"]
    df_ins["Individual_coord"] = df_ins["ProgramID"] + ":" + df["chr"].astype(str) + ":" + df["start"].astype(str) + "-" + df["end"].astype(str) + ";" + df_ins["CHR2"] + ":" + df_ins["END_INFO"] 
    df_ins["Individual_INS_LEN"] = None
    df_ins["Merged_INS_LEN"] = None
    df_ins["Program_DEL_LEN_TOTAL"] = None
    df_ins["Individual_Overlap"] = None
    df_ins["Individual_Overlap_ID"] = None
    df_ins["Program_Overlap_SUM"] = None
    ...
    merged = df.groupby(["SV_ID", "CHR2"])["ID_PROGRAM"].apply(lambda x: ','.join(x.unique())).reset_index()
    merged["SVTYPE"] = "TRA"
    ...
    merged["GT_MD"] = list(df_ins.groupby(["SV_ID", "CHR2"])["GT"].first().reset_index(drop=True))
    ...
    for program in programs:
        ...
        df_prog_temp[str(program + "_overlap_INFO")] = None
        ...
        # Inicializar las columnas vacías para los nuevos programas
    ...
    merged.drop(columns=["CHR2"], inplace=True)
